...
io.on("connect", (socket) => {
  ...
  if (statistic[`${socket.conn.remoteAddress}`]) {
    ...
    ] = `Переподключился клиент с ip адреса: ${socket.conn.remoteAddress}`;
  } else {
    statistic[
      `${socket.conn.remoteAddress}`
    ] = `Подключился клиент с ip адреса: ${socket.conn.remoteAddress}`;
  }
  socket.broadcast.emit("client_connected", [
    statistic[`${socket.conn.remoteAddress}`],
    client_count,
  ]);
  ...
  socket.on("disconnect", () => {
    ...
    statistic[
      `${socket.conn.remoteAddress}`
    ] = `Отключился клиент с ip адреса: ${socket.conn.remoteAddress}`;
    client_count--;
    console.log(client_count);
  });
