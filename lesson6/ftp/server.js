...
app.get("*", function (req, res) {
  ...
  (() => {
    if (!checkDir(newPath)) {
      start([newPath, req.url]).then((result) => res.send(result.data));
    } else {
      ...
      res.send(`<body>
				<div style="display: flex; flex-direction:column;">
				<a href='${arrPath.length > 1 ? "/" : ""}${arrPath
        .slice(0, arrPath.length - 1)
        .join("/")}/'><button>Back</button></a>
					${list
            .map((item, index) => {
              return `<a href='/${req.url
                .split("/")
                .filter(function (el) {
                  return (el != null && el != "") || el === 0;
                })
                .join(
                  "/"
                )}/${item}'><button id="${index}">${item}</button></a>`;
            })
            .join("")}
				</div>
			</body>`);
